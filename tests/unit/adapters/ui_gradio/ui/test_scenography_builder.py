...
def _base_args(**overrides: object) -> dict:
    """Return minimal valid kwargs for ScenographyFormInput."""
    defaults = {
        "description": "Element",
        "elem_type": "circle",
        "cx": 50.0,
...
def _build(**overrides: object) -> dict:
    """Build scenography data via the DTO."""
    form = ScenographyFormInput(**_base_args(**overrides))
    return build_scenography_data(form)
...
class TestBuildRect:
    def test_valid_rect(self):
        result = _build(elem_type="rect", x=10, y=10, width=50, height=30)
        assert result["ok"] is True
...
class TestBuildPolygon:
    def test_valid_polygon(self):
        ...
        result = _build(elem_type="polygon", points_data=pts)
        assert result["ok"] is True
    ...
    def test_none_points(self):
        result = _build(elem_type="polygon", points_data=None)
        assert result["ok"] is False
...
class TestBuildCommonValidation:
    ...
    def test_allow_overlap_passed_through(self):
        result = _build(allow_overlap=True)
        assert result["ok"] is True
        assert result["allow_overlap"] is True

    def test_description_stripped(self):
        result = _build(description="  Tree  ")
        assert result["ok"] is True
